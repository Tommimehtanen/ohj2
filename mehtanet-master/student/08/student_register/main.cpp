...
bool write_to_file(const std::string& filename, std::map< std::string, Student*> data)
{
...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            if(student_numbers.find(student_id) == student_numbers.end())
            {
                std::cout << "There is no student with the given number!\n"<< std::endl;
                                continue;
            }
            Student* student = student_numbers.at(student_id);
            ...
            if(!write_to_file(file_name, user_ids))
                return EXIT_FAILURE;
            for(auto pair: student_numbers) {
                pair.second = nullptr;
            }
